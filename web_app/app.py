...
app = Flask(__name__)
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=60) # 60-minute session timeout
...
def recommend():
    ...
    while len(recommendations) <= 6 and len(bars_df) > len(recommendations):
    ...
    if request.method == "POST":
        ...
        if bar_name:
            ...
            if bar_to_add:
                ...
                bars_collection.insert_one(new_bar)
                # flash(f"Added {bar_name} to your list!", "success") # ADD FLASH!!!
